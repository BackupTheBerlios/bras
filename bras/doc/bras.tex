...
%%%%%%%%%%%%%%%%%%%%%%%%
%
% $Revision: 1.33 $, $Date: 2001/12/30 09:02:10 $
%
%%%%%%%%%%%%%%%%%%%%%%%%
...
logical combination of a few predicates. The following section
...
\begin{Describe}
\item[Synopsis] \texttt{include [file join \$::bras::base cdeps.rule]}
\item[Description] Maintains a dependency cache in dependence of a
  source file.
...
\item[Command] The external command executed is
\begin{quote}\ttfamily 
  \$CC \$DEPOPTS \$INCLUDES [lindex \$deps 0]
\end{quote}
\indextt{DEPOPTS}\indextt{CC}\indextt{INCLUDES}

Set \texttt{\$DEPOPTS} to the options which instruct your C compiler
to emit dependency information for \make.
\end{Describe}
...
\item[Description] Maintains an executable in dependence of an object
  ...
  link the files rather than deriving the dependeny relation
  automatically (see the discussion in section
  ~\ref{secSuffixAsDefCmd}). The dependeny relation with all the
...
\indextt{::bras::cvsknown}\indextt{cvsknown}
\begin{Describe}
...
\item[Description] Retrieves the name of the namespace associated with
  the \brasfile\ in directory \textit{dir}. For this to work, the
...
\item[Description] Installs a file \textit{source} as a file
  ...
  \texttt{[file mkdir]}. After copying, the targets access permissions 
  are set to \textit{perm}, except if \textit{perm} is left to its
...
\label{proc:linkvar}
\begin{Describe}
\item[Synopsis] \Tt{linkvar} \It{varname} \It{dir}
\item[Description] Make the variable with name \textit{varname} an
  alias for the variable of the same name in the namespace associated
