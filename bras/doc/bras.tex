...
%%%%%%%%%%%%%%%%%%%%%%%%
%
% $Revision: 1.24 $, $Date: 2000/05/28 11:19:37 $
%
%%%%%%%%%%%%%%%%%%%%%%%%
...
\item[Name] \Flash{varchanged} --- did a given variable's value
  change?
\item[Synopsis] \Tt{varchanged} \It{varnames}
\item[Description] The predicate \texttt{varchanged} tests if any of
  ...
  Before, all given variable names considered as targets. However the
  ...
  current value with one checked previously.

  Since the variable's contents are not stored in a file,
  \texttt{varchanged} cannot communicate values it has seen between
  different invocations of \bras. Consequently, its main use is
  probably within a wish-application, were the user changes variable
  values to trigger certain events.

  However the following example describes how \texttt{varchanged} can
  still used effectively in command line invocations of \bras.
\item[Example]
  Suppose you have a parameter file to store parameters of
  ...
  second and all later considerations of 
  \texttt{[varchanged ::paramA
