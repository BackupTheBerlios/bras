...
%%%%%%%%%%%%%%%%%%%%%%%%
%
% $Revision: 1.11 $, $Date: 1997/11/09 19:44:17 $
%
%%%%%%%%%%%%%%%%%%%%%%%%
...
\section{Incompleteness Remarks}
This doc is a bit outdated. In particular the following topics do not
reflect the current state of the software:
\begin{itemize}
\item The meaning of specifying more than one rule for a target has
changed. Now only one rule is actually maintained for a
target. Specifying more than one results in an extension of the first
rule.
\item The commands \texttt{ClearCmd}, \texttt{ClearDeps},
\texttt{ClearPreqs},\texttt{GetCmd}, \texttt{GetDeps}, and
\texttt{GetPreqs} which allow rule-base surgery are not yet described.
\item The algorithm to find a matching pattern rule for a target was
changed so that it is able to chain over several pattern rules without
requiring that intermediate dependencies must exist as a file.
\item The new rule-type \texttt{CDepend} handles the trickery of
dependencies for object files created from \texttt{.c}-files. (See
\texttt{cDependRule.tcl}) 
\end{itemize}

All this new stuff is not yet really tested. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Why That?}
...
WARNING: As of 1997-11-09, a pattern rule cannot have more than one
dependency.
\end{quote}
...
Quite some people helped to push \bras{} into the right direction. I
...
\item Jason Gunthorpe \texttt{<jgg@gpu.srv.ualberta.ca>} started
experiments to get rid of the \texttt{make/automake}-combination with
\bras{}. This resulted in two things: The rule-base was made more
visible and accessible to the user, and a new type of rule was
developed to handle the dependency between \texttt{.o}- and
\texttt{.c}-files. 
\end{itemize}
